"""
Reproduce paper Tables 1, 2, 4 and Figure 5 for the Travel dataset.
Compares two LLM-generated synthetic datasets: ChatGPT vs Gemini.

Fixed splits (no randomness in data partitioning):
  travel_train.csv  — 80 % real data  (fine-tuning source for generative models)
  travel_test.csv   — 20 % real data  (held-out evaluation set, never seen by generators)

Table 1  — ML Utility: Accuracy (%)        | LR / DT / RF | Original / ChatGPT / Gemini
Table 2  — Discriminator Accuracy (%)      | RF           | ChatGPT / Gemini (lower=better)
Table 4  — ML Utility: ROCAUC + F1 (%)    | LR / DT / RF | ChatGPT / Gemini
Figure 5 — DCR distributions               | ChatGPT / Gemini vs Train, baseline = Test vs Train
...
def load_all():
    train_real = pd.read_csv(HERE / "travel_train.csv")
    test_real  = pd.read_csv(HERE / "travel_test.csv")
    syn_cg     = pd.read_csv(HERE / "travel_synthetic_chatgpt.csv")
    syn_ge     = pd.read_csv(HERE / "travel_synthetic_gemini.csv")
...
def print_table1(store, syn_names):
    print(f"\n{SEP}")
    print("TABLE 1  ML Utility — Accuracy (%)  |  Dataset: Travel")
