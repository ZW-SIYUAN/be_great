...
def main(config_path: str) -> None:
    ...
    # 所有路径相对于本脚本所在目录（normal_experiment/sick/）
    ...
    print(f"正例率 ({target}!='negative'): "
          f"{df[target].ne('negative').mean()*100:.2f}%")
    ...
    if target in synthetic_df.columns:
        print(f"正例率: {synthetic_df[target].ne('negative').mean()*100:.2f}%")
