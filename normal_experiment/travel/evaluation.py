...
Fixed splits (no randomness in data partitioning):
  travel_train.csv    — 80 % real data  (used to fine-tune GReaT)
  travel_test.csv     — 20 % real data  (held-out evaluation set)
  travel_synthetic.csv — GReaT synthetic data (same size as train set)
...
def load_all():
    ...
    syn_great  = pd.read_csv(HERE / "travel_synthetic.csv")

    # GReaT occasionally generates malformed rows (column value bleeds into next
    # column text), which causes NaN after parsing. Drop them.
    before = len(syn_great)
    syn_great = syn_great.dropna().reset_index(drop=True)
    dropped = before - len(syn_great)
    if dropped:
